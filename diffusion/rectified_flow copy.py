...
class RectifiedFlow(torch.nn.Module):
    ...
    def forward(self, x, cond, is_discrete=False):
        ...
        if isinstance(model_output, tuple):
            ...
            elif loss_stratgy_name == "Capacity_Pred_wPlr":
                layer_idx_list, ones_list, pred_c_list, prl_list, CapacityPred_loss_weight = model_output[2:]

                if len(ones_list) == 0:
                    pass
                else:
                    terms["cp_loss"] = 0
                    for layer_idx, ones, pred_c, plr_value in zip(layer_idx_list, ones_list, pred_c_list, prl_list):
                        terms[f"Capacity_Pred_loss_{layer_idx}"] = nn.BCEWithLogitsLoss()(pred_c, ones)
                        terms["loss"] +=  terms[f"Capacity_Pred_loss_{layer_idx}"]  * CapacityPred_loss_weight
                        terms["cp_loss"] += terms[f"Capacity_Pred_loss_{layer_idx}"]  * CapacityPred_loss_weight
                        ...
                        if plr_value is not None:
                            ...
                            terms["loss"] += terms[f"prl_loss_{layer_idx}"].mean()
            ...
            elif loss_stratgy_name == "RaceAll_official_bias":
                layer_idx_list, ones_list, pred_c_list, plr_value_list = model_output[2:]
                terms["loss"] = 0
                terms["cp_loss"] = 0

                for layer_idx, ones, pred_c, plr_value in zip(layer_idx_list, ones_list, pred_c_list, plr_value_list):

                    terms[f"Capacity_Pred_loss_{layer_idx}"] = nn.BCEWithLogitsLoss()(pred_c, ones)
                    terms["loss"] +=  terms[f"Capacity_Pred_loss_{layer_idx}"] * 1.0
                    terms["cp_loss"] += terms[f"Capacity_Pred_loss_{layer_idx}"] * 1.0

                    if plr_value is not None:
                        terms[f"prl_loss_{layer_idx}"] = mean_flat(((ut - plr_value) ** 2)) * 1e-2
                        terms["loss"] += terms[f"prl_loss_{layer_idx}"].mean()

            elif loss_stratgy_name == "TC_MoE":
                info_list = model_output[2]
                layer_idx_list, fi_list = info_list["layer_idx_list"], info_list["fi_list"]

                for layer_idx, fi in zip(layer_idx_list, fi_list):
                    terms[f"fi_{layer_idx}_std"] = fi.std()
                    terms[f"fi_{layer_idx}_max"] = fi.max()
                    terms[f"fi_{layer_idx}_min"] = fi.min()
    ...
    def sample(self, z, cond, null_cond=None, sample_steps=50, cfg=2.0, progress=False, mode='euler'):
        ...
        def fn_v(z, t):
            ...
            if null_cond is not None:
                ...
                print(vc.shape)
                print(vu.shape)
                cos_sim = torch.nn.functional.cosine_similarity(vc, vu)
                # print(cos_sim)
                print(((cos_sim>0.99).sum() / cos_sim.flatten().shape[0]))
                print(((cos_sim>0.999).sum() / cos_sim.flatten().shape[0]))
                mask = cos_sim.unsqueeze(1) > 0.999
                ratio = (cos_sim>0.99).sum() / cos_sim.flatten().shape[0]
                NFE += 1
                if ratio > 0.8:
                    vu = vu * mask.float()
                    vu = vc * (1 - mask.float()) + vu
                    NFE -= 1
                vc = vu + cfg * (vc - vu)
