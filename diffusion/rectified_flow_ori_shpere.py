...
def prepare_t_seq(sample_steps, device, timestep_shift=1.0, 
    ...
    if custom_t_seq is not None:
        ...
        # 默认: linear + shift
        base = torch.linspace(1, 0, sample_steps + 1, device=device) * torch.pi / 2
        t_seq = torch.tensor(
...
class RectifiedFlow(torch.nn.Module):
    ...
    def forward(self, x, cond, timestep_shift=0.1):
        ...
        t = torch.rand((b,), device=x.device) * torch.pi / 2
        ...
        sint = torch.sin(t)  # 修正：sint 应为正弦（sin）
        cost = torch.cos(t)  # cost 保持余弦（cos）
        cost_exp = cost.view([b, *([1] * len(x.shape[1:]))])
        sint_exp = sint.view([b, *([1] * len(x.shape[1:]))])

        ut = cost_exp * eps - sint_exp * z0
        zt = sint_exp * eps + cost_exp * z0
