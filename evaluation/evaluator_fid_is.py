...
def save_activations(pool_3, spatial, output_path):
    """
    将 pool_3 和 spatial 特征保存为 .npz 文件

    参数:
        pool_3 (numpy.ndarray): pool_3 特征，形状为 [N x X]
        spatial (numpy.ndarray): spatial 特征，形状为 [N x Y]
        output_path (str): 输出 .npz 文件的路径
    """
    np.savez(output_path, pool_3=pool_3, spatial=spatial)
...
def main():
    ...
    ref_stats = evaluator.read_statistics_feature(args.ref_npz)

    file_path = Path('VIRTUAL_imagenet256_labeled_Activations.npz')  # 替换为你的文件路径
    if file_path.exists():
        print(f"文件 {file_path} 存在，正在加载...")
        data = np.load(file_path)
        ref_acts = data["pool_3"]
        spatial = data["spatial"]
        print("文件内容:", list(data.keys()))  # 打印文件中的数组名称
    else:
        print(f"文件 {file_path} 不存在")
        ref_acts, spatial = evaluator.read_activations(args.ref_batch)
        output_path = 'VIRTUAL_imagenet256_labeled_Activations.npz'  # 输出文件路径
        save_activations(ref_acts, spatial, output_path)
    ...
    sample_stats, sample_acts = evaluator.read_statistics_feature(args.sample_npz)
...
class Evaluator:
    ...
    def read_statistics_feature(self, npz_path: str):
        obj = np.load(npz_path)

        if "activations" in list(obj.keys()):
            return FIDStatistics(obj["mu"], obj["sigma"]), obj["activations"]
        else:
