...
class Dinov2Decoder(pl.LightningModule):
    def __init__(self,
                 ddconfig,
                 dinoconfig,
                 lossconfig,
        ...
        self.encoder = torch.hub.load(
            repo_or_dir=dinoconfig['dinov2_location'],
            model=dinoconfig['model_name'],
            source="local",
            # weights=dinoconfig['weights'],
        ).eval()
        checkpoint = torch.load(dinoconfig['weights'], map_location="cpu")
    ...
    def encode(self, x):
        ...
        h = h.view(h.shape[0], -1, int(x.shape[2]//14), int(x.shape[3]//14)).contiguous()
        return h
