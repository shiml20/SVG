...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            DiTBlock(hidden_size, num_heads, head_dim=head_dim, mlp_ratio=mlp_ratio, use_swiglu=use_swiglu, qk_norm=qk_norm) for _ in range(depth)
        ])
        self.ln1 = nn.Linear(in_channels, hidden_size)
        # self.ln2 = nn.Linear(hidden_size, in_channels)
        # approx_gelu = lambda: nn.GELU(approximate="tanh")
        ...
        # self.repa_layer = FinalLayer(hidden_size, patch_size, self.out_channels)
        
        projector_dim = 2048
        self.projector = build_mlp(hidden_size, projector_dim, in_channels)
        # self.projectors = nn.ModuleList([
        #     build_mlp(hidden_size, projector_dim, in_channels) for z_dim in z_dims
        #     ])
    ...
    def forward(self, x, t, y):
        ...
        for i, block in enumerate(self.blocks):
            x = block(x, c)                      # (N, T, D)
            if i == 3 and self.training:
                repa_out = self.projector(x)
