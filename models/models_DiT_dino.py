...
class DiT(nn.Module):
    ...
    def forward(self, x, t, y):
        ...
        # y_mix = torch.full((x.shape[0],), 207, dtype=torch.long, device=x.device)
        # import ipdb; ipdb.set_trace()

        # if torch.sum(y) == 8000:
            # y_mix = torch.full((x.shape[0],), 1000, dtype=torch.long, device=x.device)

        y = self.y_embedder(y, self.training)    # (N, D)
        # y_mix = self.y_embedder(y_mix, self.training)  # (N, D)

        # ratio = 0.4
        # y = (1 - ratio) * y + ratio * y_mix
    ...
    def forward_with_cfg(self, x, t, y, cfg_scale):
        ...
        eps, rest = model_out[:, :channels_num], model_out[:, channels_num:]
