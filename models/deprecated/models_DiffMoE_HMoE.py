...
from .blocks_diffmoe_HMoE import *
...
class DiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, head_dim=None, mlp_ratio=4.0, 
        ...
        if "granularity" in MoE_config.keys():
            granularity = MoE_config.granularity
        else:
            granularity = 1

        self.k_ratio = [1, 1, 2, 2, 4, 4, 8, 8, 
        ...
        if use_moe:
            if use_swiglu==False:
                ...
                self.mlp = SparseMoEBlock(
                    experts=[Mlp(in_features=hidden_size, hidden_features=int(base_hidden_dim/self.k_ratio[_] ), act_layer=approx_gelu, drop=0) 
