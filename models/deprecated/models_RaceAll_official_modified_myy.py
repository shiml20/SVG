...
from .blocks_race_all_official_modified_myy import *
...
class DiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, head_dim=None, mlp_ratio=4.0, 
        ...
        base_hidden_dim = int(hidden_size * mlp_ratio)
        ...
        if use_moe:
            ...
            if use_swiglu==False:
                approx_gelu = lambda: nn.GELU(approximate="tanh")
                self.mlp = Mlp(in_features=hidden_size, hidden_features=base_hidden_dim, act_layer=approx_gelu, drop=0)
            else:
                self.mlp = MoeMLP(hidden_size=hidden_size, intermediate_size=base_hidden_dim, )
...
class DiT(nn.Module):
    ...
    def initialize_weights(self):
        ...
        def init_MLP(module):
            granularity = self.MoE_config.get("granularity", 1)
            gain = math.sqrt((granularity + 4) / (5 * granularity))
            nn.init.xavier_uniform_(module.fc1.weight, gain=gain)
            nn.init.xavier_uniform_(module.fc2.weight, gain=gain)
        ...
        for block in self.blocks:
            if self.init_MoeMLP: 
                ...
                for expert in block.mlp.experts:
                    init_MLP(expert)
                print("init MoE related module with xavier_uniform_ using Dense fan_in and fan_out")

            if self.MoE_config.get("enable_plr", True):
                nn.init.constant_(block.mlp.target_head.weight, 0)
                nn.init.constant_(block.mlp.target_head.bias, 0)
    ...
    def unpatchify(self, x, out_channels=None):
        ...
        imgs: (N, H, W, C)
        """
        c = out_channels if out_channels is not None else self.out_channels
    ...
    def forward(self, x, t, y):
        ...
        c = t + y                                # (N, D)
        router_similarity_loss_list = []
        prl_list = []
        layer_idx_list = []
        for layer_idx, block in enumerate(self.blocks):
            x, router_similarity_loss, prl = block(x, c)                      # (N, T, D)
            if router_similarity_loss is not None:
                router_similarity_loss_list.append(router_similarity_loss)
                if prl is not None:
                    prl_list.append(self.unpatchify(prl, out_channels=self.out_channels // 2))
                else:
                    prl_list.append(prl)
        ...
        return x, "RaceAll_official", layer_idx_list, router_similarity_loss_list, prl_list
