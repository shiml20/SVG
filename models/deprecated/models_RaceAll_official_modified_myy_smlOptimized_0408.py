...
from .blocks_race_all_official_modified_optimized_0408_bias import *
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, c):
        ...
        if self.use_moe:
            x_mlp, ones, pred_c, plr_value = self.mlp(modulate(self.norm2(x), shift_mlp, scale_mlp))
            x = x + gate_mlp.unsqueeze(1) * x_mlp
            return x, ones, pred_c, plr_value
        else:
...
class DiT(nn.Module):
    ...
    def forward(self, x, t, y):
        ...
        for layer_idx, block in enumerate(self.blocks):
            x, ones, pred_c, plr = block(x, c)                      # (N, T, D)
            if ones is not None:
                ones_list.append(ones)
                pred_c_list.append(pred_c)
                if plr is not None:
                    plr_list.append(self.unpatchify(plr, out_channels=self.out_channels // 2))
                else:
                    plr_list.append(plr)
        ...
        return x, "RaceAll_official_bias", layer_idx_list, ones_list, pred_c_list, plr_list
