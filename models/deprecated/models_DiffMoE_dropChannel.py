...
class PercentMask(nn.Module):
    def __init__(self, percent=0.1, mode='hard'):
        super().__init__()
    ...
    def forward(self, x):
        ...
        Returns:
            mask: tensor of same shape as x with last N% positions masked
        """
        batch_size, seq_len, dim = x.shape
        n_mask = int(seq_len * self.percent)
        
        # Create base mask (1=keep, 0=mask)
        mask = torch.ones((batch_size, 1, dim)).to(x)
        mask[:, :, -n_mask:] = 0  # Mask last N% positions
        
        if self.mode == 'soft':
            # Soft mask (linear decay)
            ramp = torch.linspace(1, 0, n_mask, device=x.device)
            mask[:, :, -n_mask:] = ramp.view(1, n_mask, 1)
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, c):
        ...
        mask = PercentMask(percent=0.2, mode='hard')(x) 
