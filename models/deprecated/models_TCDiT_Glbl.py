...
class MoEGate(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if self.training:
            ...
            return topk_idx, topk_weight, None, None, None
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, c):
        ...
        if self.use_moe:
            x_mlp, fi, Pi  = self.mlp(modulate(self.norm2(x), shift_mlp, scale_mlp))
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        # self.balance_loss_weight = MoE_config.balance_loss_weight
        ...
        # self.blocks = nn.ModuleList([
            # DiTBlock(hidden_size, num_heads, mlp_ratio, num_experts, num_experts_per_tok, 
            # num_shared_experts, pretraining_tp, use_swiglu, MoE_config, use_flash_attn) for _ in range(depth)
        # ])
