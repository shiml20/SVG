...
from .blocks_diffmoe_plr import *
...
class DiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, head_dim=None, mlp_ratio=4.0, 
        ...
        granularity = MoE_config.get("granularity", 1)
    ...
    def forward(self, x, c):
        ...
        if self.use_moe:
            x_mlp, ones, pred_c, plr = self.mlp(modulate(self.norm2(x), shift_mlp, scale_mlp))
            x = x + gate_mlp.unsqueeze(1) * x_mlp
            return x, ones, pred_c, plr
        else:
...
class DiT(nn.Module):
    ...
    def forward(self, x, t, y):
        ...
        for layer_idx, block in enumerate(self.blocks):
            x, ones, pred_c, prl = block(x, c)                      # (N, T, D)
            if ones is not None:
        ...
        return x, "Capacity_Pred_wPlr", layer_idx_list, ones_list, pred_c_list, prl_list, self.CapacityPred_loss_weight
