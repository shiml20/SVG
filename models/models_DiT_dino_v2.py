...
class DimProjector(nn.Module):
    """复杂的维度变换器：Input -> Hidden -> Hidden -> Output"""
    def __init__(self, in_dim, out_dim, hidden_dim=None, dropout=0.1):
        super().__init__()
        if hidden_dim is None:
            hidden_dim = max(in_dim, out_dim) * 2  # 自动放大中间层
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        # 复杂版 ln1: input -> hidden
        self.ln1 = DimProjector(input_size, hidden_size)

        # 复杂版 ln2: hidden -> input
        self.ln2 = DimProjector(hidden_size, input_size)
