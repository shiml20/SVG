...
class SparseMoEBlock(nn.Module):
    ...
    def __init__(self, experts, hidden_dim, num_experts, n_shared_experts=0, capacity=2, mlp_ratio=4.0):
        ...
        # expert_threshold = torch.tensor([0]*num_experts)
        # expert_threshold = expert_threshold.unsqueeze(0).unsqueeze(0)
    ...
    def forward_train(self, x):
        ...
        x_world = [torch.empty_like(x) for _ in range(world_size)]
        dist.all_gather(x_world, x)
        x_world = torch.cat(x_world, dim=0)  # (S_world, D)
        S_world = x_world.shape[0]  # Total number of tokens across all ranks
        S_per_rank = S_world // world_size  # Number of tokens per rank

        # Compute scores on all tokens
        logits = F.linear(x_world, self.gate_weight, None)  # (S_world, num_experts)
        scores = logits.softmax(dim=-1).permute(1, 0)  # (num_experts, S_world)

        # Compute k based on current rank's tokens
        S = S_per_rank * world_size  # Total tokens
        k_total = int((S / self.num_experts) * self.capacity)  # Total tokens per expert across all ranks
        k_per_rank = k_total // world_size  # Number of tokens per expert per rank

        # Perform topk on the global scores
        gating, index = torch.topk(scores, k=k_total, dim=-1, sorted=False)  # (num_experts, k_total)

        # Determine the indices of the current rank's tokens in the global scores
        current_rank_start = local_rank * S_per_rank
        current_rank_end = (local_rank + 1) * S_per_rank

        # Filter out the indices that belong to the current rank
        # mask = (index >= current_rank_start) & (index < current_rank_end)
        # index_cur_rank = index[mask] - current_rank_start  # (num_experts_per_rank,)
        # gating_cur_rank = gating[mask]  # (num_experts_per_rank,)

        # Filter out the indices that belong to the current rank
        mask = (index >= current_rank_start) & (index < current_rank_end)  # (num_experts, k_total)

        # Use torch.where to keep the structure and set invalid indices to -1
        index_cur_rank = torch.where(mask, index - current_rank_start, -1)  # (num_experts, k_total)
        gating_cur_rank = torch.where(mask, gating, 0.0)  # (num_experts, k_total)
        # print(local_rank)
        # print((index_cur_rank == -1).sum())
        # import pdb; pdb.set_trace()

        # Now, we need to extract the valid entries while keeping the structure
        # Find the valid indices in the flattened version
        valid_indices_list = [(index_cur_rank[i][None, ...] >= 0).nonzero(as_tuple=True) for i in range(self.num_experts)]
        # Gather the valid entries
        index_cur_rank_list = [index_cur_rank[i][None, ...][valid_indices_list[i]] for i in range(self.num_experts)]
        gating_cur_rank_list = [gating_cur_rank[i][None, ...][valid_indices_list[i]] for i in range(self.num_experts)]

        y = torch.zeros_like(x, dtype=x.dtype)
        ones = torch.zeros((self.num_experts, S_per_rank), dtype=x.dtype, device=x.device)
        capacity_pred = self.capacity_predictor(x.detach())
        

        for i, expert in enumerate(self.experts): 
            y[index_cur_rank_list[i], :] += gating_cur_rank_list[i].unsqueeze(-1) * expert(x[index_cur_rank_list[i], :])
            ones[i][index_cur_rank_list[i]] = 1.
    ...
    def forward_train_opt_vallina(self, x):
        B, s, D = x.shape

        local_rank = get_local_rank()
        world_size = dist.get_world_size()
        print("Current Rank", local_rank)
        ...
        # # Vectorize the expert computation using vmap

        # GPU2 A800 1.52 steps/s group_num 2
    ...
    def forward_train_optv2(self, x):
        B, s, D = x.shape
        identity = x
        ...
        # Vectorize the expert computation using vmap
        # expert_outputs = vmap(lambda expert, inputs: expert(inputs))(self.experts, expert_inputs)
        # Compute expert outputs in parallel
        expert_outputs = torch.stack([expert(expert_inputs[i]) for i, expert in enumerate(self.experts)])  # (num_experts, k, D)

        # 使用 vmap 对所有专家进行并行计算
        # expert_outputs = vmap(self.apply_expert)(self.experts, expert_inputs)
        ...
        # BUG y = y.view(S, self.num_experts, D).sum(dim=0, keepdim=False)  # (S, 1, D)
    ...
    def forward_train_vallina(self, x):
        B, s, D = x.shape
        identity = x
        ...
        gating, index = torch.topk(scores, k=k, dim=-1, sorted=False)   # num_experts, k
        ...
        for i, expert in enumerate(self.experts): 
            ...
            y[index[i], :] += gating[i].unsqueeze(-1) * expert(x[index[i], :])
            ones[i][index[i]] = 1.
    ...
    def forward_eval_fixk(self, x):
        ...
        # Fixed capacity for each expert
        k_fixed = int((S / self.num_experts) * self.capacity)
        ...
        # Get top-k gating values and indices for each expert
        gating, index = torch.topk(scores, k=k_fixed, dim=-1, sorted=False)  # gating: (num_experts, k), index: (num_experts, k)
        ...
        if not debug:
            ...
            # offset = torch.arange(0, self.num_experts).unsqueeze(1) * 1.0
            # self.register_buffer('offset', offset)
            # 在初始化时对所有专家的权重和偏置进行堆叠
            self.fc1_weights = torch.stack([expert.fc1.weight for expert in self.experts])
            self.fc1_biases = torch.stack([expert.fc1.bias for expert in self.experts])
            self.fc2_weights = torch.stack([expert.fc2.weight for expert in self.experts])
            self.fc2_biases = torch.stack([expert.fc2.bias for expert in self.experts])

            # 根据索引选择对应的专家参数
            selected_fc1_weights = self.fc1_weights
            selected_fc1_biases = self.fc1_biases

            selected_fc2_weights = self.fc2_weights
            selected_fc2_biases = self.fc2_biases

            # (num_experts, k, D)
            # (num_experts, mlp_ratio * D, D)

            # 第一层计算: inputs -> hidden
            hidden = torch.bmm(expert_inputs, selected_fc1_weights.transpose(-1, -2))
            # import pdb; pdb.set_trace()
            hidden = hidden + selected_fc1_biases.unsqueeze(1)


            # hidden = torch.bmm(selected_fc1_weights, expert_inputs.transpose(-1, -2))
            # hidden = hidden + selected_fc1_biases.unsqueeze(-1)

            approx_gelu = lambda: nn.GELU(approximate="tanh")
            hidden = approx_gelu()(hidden)
            # hidden = F.gelu(hidden)  # 激活函数

            # # 手动生成 Dropout 掩码并应用
            # if self.training:  # 仅在训练模式下应用 Dropout
            #     dropout_mask1 = (torch.rand(hidden.shape) > self.drop_prob).float().to(hidden.device)
            #     hidden = hidden * dropout_mask1 / (1 - self.drop_prob)

            # # Norm 层（如果有的话）
            # if self.norm_weights is not None:
            #     print("here")
            #     self.norm_weights = self.norm_weights.to(x.device)
            #     self.norm_biases = self.norm_biases.to(x.device)
            #     selected_norm_weights = self.norm_weights
            #     selected_norm_biases = self.norm_biases
            #     mean = hidden.mean(dim=-1, keepdim=True)
            #     var = hidden.var(dim=-1, keepdim=True, unbiased=False)
            #     hidden = (hidden - mean) / torch.sqrt(var + 1e-5)
            #     hidden = hidden * selected_norm_weights + selected_norm_biases

            # 第二层计算: hidden -> output
            # (num_experts, k, mlp_ratio * D)
            # (num_experts, mlp_ratio * D, D)

            expert_outputs = torch.bmm(hidden, selected_fc2_weights.transpose(-1, -2))
            expert_outputs = expert_outputs + selected_fc2_biases.unsqueeze(1)


            # expert_outputs = torch.bmm(selected_fc2_weights, hidden.transpose(-1, -2))
            # expert_outputs = expert_outputs + selected_fc2_biases.unsqueeze(-1)


            # # 手动生成 Dropout 掩码并应用
            # if self.training:  # 仅在训练模式下应用 Dropout
            #     dropout_mask2 = (torch.rand(outputs.shape) > self.drop_prob).float().to(outputs.device)
            #     expert_outputs = expert_outputs * dropout_mask2 / (1 - self.drop_prob)
        ...
        # Scatter the gated outputs back to the full output tensor
        # for i in range(self.num_experts):
            # y[index[i]] += gated_outputs[i]  # Accumulate expert outputs
        ...
        y = torch.scatter(
            y,  # Target tensor of shape [S * num_experts, D]
            0,  # Dimension to scatter along
            index.unsqueeze(1).repeat(1, D),  # Indices of shape [num_experts * k, D]
            gated_outputs_flat  # Source values of shape [num_experts * k, D]
        )
        ...
        # Sum the outputs from all experts
        y = y.view(self.num_experts, S, D).sum(dim=0, keepdim=False)  # (S, 1, D)
        # y = y.view(S, self.num_experts, D).sum(dim=1, keepdim=False)  # (S, 1, D)
        
        # BUG y = y.view(S, self.num_experts, D).sum(dim=1, keepdim=False)  # (S, 1, D)
    ...
    def forward_eval(self, x):
        ...
        capacity_pred = self.capacity_predictor(x.detach())
        # capacity_pred = F.softmax(capacity_pred)
        capacity_pred = F.sigmoid(capacity_pred)
        # print(capacity_pred)
        # capacity_pred = F.sigmoid(capacity_pred) > 0.5

        # k_fixed = int((S/self.num_experts) * self.capacity)
        # GlobalBatchSize / #GPU * 256 / #Experts *CapacityRatio
        ...
        logits = F.linear(x, self.gate_weight, None)    # bs*seq_len, num_experts
        scores = logits.softmax(dim=-1).permute(-1, -2)     #  num_experts, bs*seq_len
        # gating, index = torch.topk(scores, k=k_fixed, dim=-1, sorted=False)   # num_experts, k
        ...
        for i, expert in enumerate(self.experts): 
            ...
            # k_fixed = torch.where(capacity_pred[:,i]>0.2, 1, 0).sum()
        ...
        # for i, expert in enumerate(self.experts):
        #     # 计算 k1 和 k2
        #     k1 = torch.where(capacity_pred[:, i] > 0.9999, 1, 0).sum().item()  # k1 是 capacity_pred > 0.5 的 token 数量
        #     k2 = torch.where(capacity_pred[:, i] > 1, 1, 0).sum().item()  # k2 是 capacity_pred < 1 的 token 数量
        #     # print(k2, k1)
        #     # 如果 k1 == k2，跳过当前专家
        #     if k1 == k2:
        #         continue

        #     # 选择 topk2 的 token，并确保排序
        #     gating_all, index_all = torch.topk(scores[i], k=k1, dim=-1, sorted=True)
        #     # print(index_all)
        #     # 截取从 topk1 到 topk2 的部分（已经是有序的）
        #     gating = gating_all[k2:k1]
        #     index = index_all[k2:k1]
...
class DiT(nn.Module):
    ...
    def forward(self, x, t, y):
        ...
        if self.training and self.capacity_schedule:
            ...
            # print(num_experts, capacity, capacity_schedule_stage_I_iters, capacity_schedule_stage_II_iters, self.training_iters)
