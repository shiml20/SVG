...
class SparseMoEBlock(nn.Module):
    ...
    def forward_train(self, x):
        ...
        print(self.expert_bias)
        print(logits)
        scores = logits.softmax(dim=0)  # (num_experts, S)
        value, index = torch.topk(scores.flatten(), k=k, dim=-1, sorted=False)
        index_expert = index // S
        index_token = index % S
        ...
        num_token_each_expert = torch.tensor([len(idx_expert) for idx_expert in index_list])


        print(num_token_each_expert)
        ...
        for expert_idx, expert in enumerate(self.experts):
            ...
            # idx_token = index_token[expert_indices]
            # x_in = x[idx_token]
            # gating_value = scores[expert_idx, idx_token]
            expert_out = expert(x_in)
            output.index_add_(
                0, 
                idx_token, 
        ...
        return x_out, token_each_expert, self.expert_bias
