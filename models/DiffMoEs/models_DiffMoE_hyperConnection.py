...
class SparseMoEBlock(nn.Module):
    ...
    def forward(self, x, hyper_connection_x):
        if self.training:
            return self.forward_train(x, hyper_connection_x)
        else:
            return self.forward_eval(x, hyper_connection_x)
    ...
    def forward_train(self, x, hyper_connection_x):
        # 记录原始输入的序列长度
        original_s_x = x.size(1)
        original_s_hyper = hyper_connection_x.size(1)
        # 合并输入（假设在序列维度dim=1拼接）
        x = torch.cat([x, hyper_connection_x], dim=1)
        B, s, D = x.shape  # s = original_s_x + original_s_hyper
        # 保留identity用于后续处理
        identity = x
        ...
        k = int((S/self.num_experts) * self.capacity /2)
        ...
        x_out = y.view(B, s, D)
        # 拆分输出为原始的两部分
        x_out_x = x_out[:, :original_s_x, :]
        x_out_hyper = x_out[:, original_s_x:, :]
        # 如果后续需要返回ones和capacity_pred，按原始逻辑处理
        ones = mask.permute(1, 0).view(B, s, self.num_experts)[:, :original_s_x, :]
        capacity_pred = capacity_pred.view(B, s, self.num_experts)[:, :original_s_x, :]
        # 返回拆分后的结果（根据需求调整返回项）
        return x_out_x, ones, capacity_pred



    def forward_eval(self, x, hyper_connection_x):
        B, s, D = x.shape
        identity = x
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, c, hyper_connection_x):
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(c).chunk(6, dim=1)
        x = x + gate_msa.unsqueeze(1) * self.attn(modulate(self.norm1(x), shift_msa, scale_msa))
        if self.use_moe:
            x_mlp, ones, pred_c = self.mlp(modulate(self.norm2(x), shift_mlp, scale_mlp), hyper_connection_x)
...
class DiT(nn.Module):
    ...
    def forward(self, x, t, y):
        ...
        hyper_connection_x = x
        for layer_idx, block in enumerate(self.blocks):
            x, ones, pred_c = block(x, c, hyper_connection_x)                      # (N, T, D)
            if ones is not None:
