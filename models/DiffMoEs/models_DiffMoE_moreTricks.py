...
class SparseMoEBlock(nn.Module):
    ...
    def __init__(self, experts, hidden_dim, num_experts, n_shared_experts=0, 
        ...
        self.max_capacity = capacity * 2  # 动态容量上限
        ...
        # 层次化门控权重
        self.primary_gate_weight = nn.Parameter(torch.empty(num_experts, hidden_dim))
        nn.init.normal_(self.primary_gate_weight, std=0.006)
    ...
    def forward_train(self, x):
        ...
        # 动态容量预测
        capacity_pred = self.capacity_predictor(x_flat)  # [B*s, num_experts]
        k = (capacity_pred * self.max_capacity).long().clamp(min=1)  # 动态k值
        
        # 多头门控计算
        all_logits = [gate(x_flat) for gate in self.gates]
        logits = torch.stack(all_logits).mean(dim=0)  # 平均多门控结果
        
        # 噪声注入增强探索
        noise = torch.randn_like(logits) * self.noise_factor
        logits = logits + noise
        
        # 层次化专家选择
        primary_logits = F.linear(x_flat, self.primary_gate_weight)
        primary_experts = torch.topk(primary_logits, 2, dim=-1).indices  # 初级筛选2个专家
        mask = torch.zeros_like(logits).scatter_(-1, primary_experts, 1)
        logits = logits * mask  # 仅在初级专家中做最终选择
        
        # 专家分配
        scores = logits.softmax(dim=-1).permute(1, 0)  # [num_experts, B*s]
        selected = []
        for i in range(self.num_experts):
            # 每个专家独立选择样本
            k_i = k[:, i].clamp(min=1).max().item()  # 取当前批次最大k值
            values, indices = torch.topk(scores[i], k=k_i, sorted=False)
            selected.append((values, indices))
        
        # 专家计算结果聚合
        y = torch.zeros_like(x_flat)
        ones = torch.zeros((self.num_experts, S), device=x.device)
        
        for expert_idx, expert in enumerate(self.experts):
            values, indices = selected[expert_idx]
            if len(indices) > 0:
                selected_x = x_flat[indices]
                expert_out = expert(selected_x)
                # 加权聚合
                y[indices] += values.unsqueeze(-1) * expert_out
                ones[expert_idx][indices] = 1.0
        ...
        # 更新动态阈值
        self.update_threshold(capacity_pred.view(-1, self.num_experts))
