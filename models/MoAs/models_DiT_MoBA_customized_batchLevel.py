...
class MobaAttention(nn.Module):
    ...
    def moba_attention(
        ...
        # key_gate = key_gate.view(batch_size, num_blocks, num_heads, head_dim) # [b, num_blocks, h, d]
        
        # key_gate = []
        # for i in range(num_blocks):
        #     start = i * self.moba_chunk_size
        #     end = min(seq_len, start + self.moba_chunk_size)
        #     key_gate.append(k_[:, :, start:end, :].mean(dim=2))
            
        # key_gate = torch.cat(key_gate, dim=0).view(batch_size, num_blocks, num_heads, head_dim)  # [b, num_blocks, h, d]
        
        # import ipdb; ipdb.set_trace()
        # 计算门控分数 s: sequence length, h: num_heads, d: head_dim, n: num_blocks
...
class DiTBlock(nn.Module):
    ...
    """
    def __init__(self, hidden_size, num_heads, head_dim=None, mlp_ratio=4.0, use_moba=True, use_swiglu=False, moba_chunk_size=16, moba_topk=4, **block_kwargs):
        super().__init__()
        self.norm1 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        if use_moba==False:
            self.attn = Attention(hidden_size, num_heads=num_heads, qkv_bias=True, **block_kwargs)
        else:
            self.attn = MobaAttention(hidden_size, num_heads=num_heads, head_dim=head_dim, qkv_bias=True, moba_chunk_size=moba_chunk_size, moba_topk=moba_topk, **block_kwargs)
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            DiTBlock(hidden_size, num_heads, head_dim=head_dim, mlp_ratio=mlp_ratio, use_swiglu=use_swiglu, use_moba=_, moba_chunk_size=moba_block_size, moba_topk=moba_topk) for _ in use_moba_list
        ])
