...
class MoHAttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn_experts = nn.ModuleList(attn_experts)
        self.gate_weight = nn.Parameter(torch.empty((num_attn_experts, dim)))

    def forward_ori(self, x: torch.Tensor) -> torch.Tensor:
        B, S, D = x.shape
        # import ipdb; ipdb.set_trace()

        k = int(self.capacity * S / self.num_attn_experts)
        logits = F.linear(x, self.gate_weight, None)  # (B, S, num_experts)
        scores = logits.softmax(dim=-1).permute(0, 2, 1)  # (num_experts, S)
        gating, index = torch.topk(scores, k=k, dim=-1, sorted=False)  # gating: (B, num_experts, k), index: (B, num_experts, k)
        y = torch.zeros_like(x, dtype=x.dtype)

        # 遍历每个专家
        for i in range(self.num_attn_experts):
            # 获取当前专家的索引
            index_i = index[:, i]  # 形状: (B, k)
            # 将索引扩展为 (B, k, D) 以匹配 y 的维度
            index_i_expanded = index_i.unsqueeze(-1).expand(-1, -1, D)
            select_x = torch.gather(x, dim=1, index=index_i_expanded)
            # 假设 expert 是当前专家的计算结果，形状为 (B, k, D)
            expert_output = gating[:, i].unsqueeze(-1) * self.attn_experts[i](select_x)  # 这里替换为你的实际计算逻辑
            # 使用 scatter_add_ 将 expert_output 累加到 y 中
            y.scatter_add_(dim=1, index=index_i_expanded, src=expert_output)
    ...
    def forward(self, x):
        ...
        # Predict capacity
        k = int((S/self.num_attn_experts) * self.capacity)
        ...
        mask = torch.zeros((self.num_attn_experts, S), dtype=x.dtype, device=x.device)
        ...
        expert_outputs = torch.stack([expert(expert_inputs[i]) for i, expert in enumerate(self.attn_experts)])  # (num_experts, k, D)
        ...
        y = torch.zeros((S * self.num_attn_experts, D), dtype=x.dtype, device=x.device)
        offset = torch.arange(0, self.num_attn_experts).unsqueeze(1).to(device=x.device) * S  # (num_experts, 1)
        ...
        y = y.view(self.num_attn_experts, S, D).sum(dim=0, keepdim=False)  # (S, 1, D)
        ...
        ones = mask.permute(1, 0).view(B, s, self.num_attn_experts)
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.q = MoHAttentionBlock(dim, 
            attn_experts = [nn.Linear(dim, self.head_dim * self.num_heads * 1, bias=qkv_bias) for _ in range(MoA_config.num_attn_experts)],
            num_attn_experts = MoA_config.num_attn_experts,
            capacity = MoA_config.capacity
        )
