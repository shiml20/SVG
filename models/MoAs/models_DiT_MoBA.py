...
class MobaAttention(nn.Module):
    ...
    def moba_attention(
        ...
        gate = torch.einsum("bhsd,bnhd->bhsn", q_.float(), key_gate.float())
...
class DiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, head_dim=None, mlp_ratio=4.0, use_swiglu=False, **block_kwargs):
        ...
        self.attn = MobaAttention(hidden_size, num_heads=num_heads, head_dim=head_dim, qkv_bias=True, **block_kwargs)
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            DiTBlock(hidden_size, num_heads, head_dim=head_dim, mlp_ratio=mlp_ratio, use_swiglu=use_swiglu, moba_chunk_size=moba_block_size, moba_topk=moba_topk) for _ in range(depth)
        ])
